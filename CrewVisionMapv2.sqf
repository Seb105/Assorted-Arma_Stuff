		...
		[_parentVehicle,_role,_unit,_turret] spawn {
			...
			while {true} do {
				...
				if (player isEqualTo gunner _parentVehicle) then {
						...
						// this checks for object intersect
						_gunnerIntersectingObject = lineIntersectsObjs [(eyePos player),(ATLtoASL _gunnerTarget), objNull, _parentVehicle, false, 18];
						if !(count _gunnerIntersectingObject == 0) then {_gunnerTarget = getposATL (_gunnerIntersectingObject select 0);};
				...
				if (player isEqualTo commander _parentVehicle) then {
						...
						// this checks for object intersect
						_commanderIntersectingObject = lineIntersectsObjs [(eyePos player),(ATLtoASL _commanderTarget), objNull, _parentVehicle, false, 18];
						if !(count _commanderIntersectingObject == 0) then {_commanderTarget = getposATL (_commanderIntersectingObject select 0);};
